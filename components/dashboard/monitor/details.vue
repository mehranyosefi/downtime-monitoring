...
<template>
  <div>
    <PrimeCard>
      <template #content>
        <div>
          ...
          <label
            for="monitor-url"
            v-text="t('phrases.etc.urlToMonitor')"
          ></label>
          <PrimeInputText
          ...
          <label
            for="monitor-url"
            v-text="t('phrases.etc.howNotifyYou')"
          ></label>
    ...
    >
      <template #content>
        <div>
          <label for="monitor-url" class="capitalize">
            <span
              v-if="locale === 'en'"
              v-text="`${t('general.monitor')} ${t('general.interval')}`"
            ></span>
            <span
              v-else
              v-text="`${t('general.interval')} ${t('general.monitor')}`"
            ></span>
          </label>
          <p class="text-xs mt-1">
            <span v-text="t('phrases.etc.monitorInterval')"></span>
            <span
              class="underline font-extrabold text-sm mx-1"
              v-text="formData.monitorIterval"
            ></span>
            <span
              v-text="
                formData.monitorIterval === 1
                  ? t('general.minute')
                  : t('general.minutes')
              "
            ></span>
          ...
          <PrimeSlider
            v-model="formData.monitorIterval"
            :pt="{
              root: 'mt-4 h-2!',
              range: 'rounded-2xl!',
            }"
